...
type Tweet struct {
	...
	UserName       string    `orm:"column(UserName)" description:"ユーザーネーム" json:"text"`
	UserScreenName string    `orm:"column(UserScreenName)" description:"ユーザースクリーンネーム" json:"text"`
	CreatedAt      time.Time `orm:"column(created_at);type(datetime)" description:"ツイート作成日時" json:"-"`
	CreatedOn      time.Time `orm:"auto_now_add;column(created_on);type(datetime)" description:"作成日時" json:"-"`
